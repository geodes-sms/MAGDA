...
public class AssociationsFactory {
	...
	public static void removeAssociationFromCash(String source, String target) {
		HashMap<String, List<String>> map = Services.relatedAssociations;
		for (List<String> values : map.values()) {
			values.removeIf(s -> (s.split("=>")[1].toLowerCase().equalsIgnoreCase(source)
					&& s.split("between")[1].split("=>")[0].toLowerCase().equalsIgnoreCase(target)));
