	...
	public Object execute(ExecutionEvent event) throws ExecutionException {
		...
		if (Services.mode == Mode.assessAtEnd) {
			try {
					...
					if (result != null) {
						for (int i = 0; i < result.length; i++) {
							if (!services.containsIgnoreCase(classesInModel, (String) result[i])) {
								...
								Services.loggerServices.info("Accept Class From list {" + (String) result[i] + "}");
					...
					while (typeAttributes.size() != 0 && cancelPressed == false) {
						...
						while (iter.hasNext()) {
							...
							if (key.trim().isEmpty() || value.trim().isEmpty()) {
								iter.remove();
								if (typeAttributes.containsKey(key)) {
									typeAttributes.remove(key);
								}
								...
								} else if (typeAttributes.containsKey(key)) {
									iter.remove();
									typeAttributes.remove(key);
								}
							}
						...
						if (result != null) {
							for (int i1 = 0; i1 < result.length; i1++) {
								...
								Services.loggerServices.info("Accept Attributes From list {" + res + "}");
				...
				while (items.size() > 0 && (items.get(0) != null) && cancelPressed == false) {
					ElementListSelectionDialog dialog = new ElementListSelectionDialog(
							Display.getCurrent().getActiveShell(), new LabelProvider());
					...
					if (result != null) {
						...
						for (int j = 0; j < result.length; j++) {
							...
							Services.loggerServices.info("Accept Association From list {" + Type +"; "+ Name+" ;"+ Target +" ; " +Source + "}");
